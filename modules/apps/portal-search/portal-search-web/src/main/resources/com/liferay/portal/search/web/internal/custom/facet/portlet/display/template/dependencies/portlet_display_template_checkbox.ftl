...
>
	...
	>
		...
		<ul class="list-unstyled">
							...
							<label class="facet-checkbox-label" for="${namespace}${entry.getBucketText()}">
								...
								<span class="custom-control-label term-name ${(entry.isSelected())?then('facet-term-selected', 'facet-term-unselected')}">
									<span class="custom-control-label-text">
										<#if entry.isSelected()>
											<strong><@liferay_ui["message"] arguments="${entry.getRangeInputs()}" key="${htmlUtil.escape(entry.getBucketText())}" /></strong>
										<#else>
											<@liferay_ui["message"]
												arguments="${entry.getRangeInputs()}"
												key="${htmlUtil.escape(entry.getBucketText())}"
											/>
										</#if>
									</span>
								</span>
			...
			<#if customFacetDisplayContext.showInputRange()>
				<li class="facet-value">
					<div class="custom-checkbox custom-control">
				...
				<div class="${(!customFacetCalendarDisplayContext.isSelected())?then("hide", "")} date-custom-range" id="${namespace}customRange">
					<div class="col-md-6" id="${namespace}customRangeFrom">
						<@liferay_aui["field-wrapper"] label="from">
							<@liferay_ui["input-date"]
								cssClass="custom-facet-custom-range-input-date-from"
								dayParam="fromDay"
					...
					<div class="col-md-6" id="${namespace}customRangeTo">
						<@liferay_aui["field-wrapper"] label="to">
							<@liferay_ui["input-date"]
								cssClass="custom-facet-custom-range-input-date-to"
								dayParam="toDay"
					...
					<@clay["button"]
						cssClass="custom-facet-custom-range-filter-button"
				...
				<div class="${(!customFacetRangeDisplayContext.isSelected())?then("hide", "")} custom-range" id="${namespace}customRange">
					<div class="col-md-6" id="${namespace}customRangeFrom">
						<div class="form-group">
							<label for="${namespace}fromInput">From</label>

							<input
								class="form-control"
								id="${namespace}fromInput"
								value='${(customFacetRangeDisplayContext.getFromValue())!""}'
								type="text"
							/>
						</div>
					</div>
					...
					<div class="col-md-6" id="${namespace}customRangeTo">
						<div class="form-group">
							<label for="${namespace}toInput">To</label>

							<input
								class="form-control"
								id="${namespace}toInput"
								value='${(customFacetRangeDisplayContext.getToValue())!""}'
								type="text"
							/>
						</div>
					</div>
					...
					<@clay["button"]
						cssClass="custom-facet-custom-range-filter-button"
						disabled=customFacetRangeDisplayContext.isRangeBackwards()
						displayType="secondary"
						id="${namespace + 'searchCustomRangeButton'}"
						label="search"
						name="${namespace + 'searchCustomRangeButton'}"
