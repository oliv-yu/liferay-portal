...
<div class="task-action">
	<aui:form action="<%= hideURL %>" method="post" name="hideFm" onSubmit="event.preventDefault();">
	...
	if (done) {
			...
			function(event) {
				A.io.request(
					...
					{
						form: {
							id: '<portlet:namespace />hideFm'
						},
						method: 'POST',
						on: {
							success: function() {
								Liferay.Util.getOpener().<portlet:namespace />refreshPortlet('<%= redirect.toString() %>');
								Liferay.Util.getWindow('<portlet:namespace />hideResultDialog').destroy();
							}
						}
					}
				);
