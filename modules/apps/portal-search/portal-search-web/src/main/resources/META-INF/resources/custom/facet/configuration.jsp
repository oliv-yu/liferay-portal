			...
			<c:otherwise>
				...
				>
					...
					<aui:select helpMessage="order-by-help" label="order-by" name="<%= PortletPreferencesJspUtil.getInputName(CustomFacetPortletPreferences.PREFERENCE_KEY_ORDER) %>" value="<%= customFacetPortletPreferences.getOrder() %>" id="preferenceKeyOrder">
						...
						<c:if test="<%= customFacetPortletPreferences.getAggregationType().equals("dateRange") || customFacetPortletPreferences.getAggregationType().equals("numericRange") %>">
							<aui:option label="ranges-configurations" value="ranges"/>
						</c:if>
					</aui:select>
				...
				>
					...
					<aui:input label="show-input-range" name="<%= PortletPreferencesJspUtil.getInputName(CustomFacetPortletPreferences.PREFERENCE_KEY_SHOW_INPUT_RANGE) %>" type="checkbox" value="<%= customFacetPortletPreferences.isShowInputRange() %>" />
				</liferay-frontend:fieldset>
...
<aui:script>
	const aggregationTypeElement = document.getElementById('<portlet:namespace />aggregationType');

	const preferenceKeyOrderElement = document.getElementById('<portlet:namespace />preferenceKeyOrder');

	var rangeOption = document.createElement("option");
	rangeOption.textContent = Liferay.Language.get('ranges-configurations');
	rangeOption.value = "ranges";

	function <portlet:namespace />aggregationSelectBox(event) {
		const currentValue = event.target.value;

		if (currentValue === 'dateRange' || currentValue === 'numericRange') {
			if (preferenceKeyOrderElement.options.length === 4) {
				preferenceKeyOrderElement.add(rangeOption);
			}
		}
		else {
			const rangesOptionItem = preferenceKeyOrderElement.options[4];
	...
	aggregationTypeElement.addEventListener('change', <portlet:namespace />aggregationSelectBox);
</aui:script>